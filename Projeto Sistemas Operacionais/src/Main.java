...
public class Main {
    ...
    public static void popular_processos(int[] tempo_execucao, int[] tempo_espera, int[] tempo_restante, int[] tempo_chegada, int[] prioridade) {
        ...
        System.out.println("A população dos processos será aleatória?");
        ...
        for (int i = 0; i < n_processos; i++) {
            if (aleatorio == 1) {
                tempo_execucao[i] = random.nextInt(1, 10);
                tempo_chegada[i] = random.nextInt(1, 10);
                prioridade[i] = random.nextInt(1, 10);
            } else {
